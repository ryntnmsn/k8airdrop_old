                                        ...
                                        <div class="flex flex-wrap items-center ps-4 rounded gap-4">
                                            @foreach($question->choices as $choice)
                                                <div class="flex-none border border-slate-700 px-4 py-2">
                                                    <input id="choices[{{$question->id}}]" type="radio" value="{{$choice->id}}" name="choices[{{$question->id}}]" class="w-4 h-4 text-indigo-600 bg-gray-100 border-gray-300 focus:ring-indigo-500 dark:focus:ring-indigo-600 focus:ring-2">
                                                    <label for="choices[{{$question->id}}]" class="w-full py-4 ms-2 text-sm font-medium text-gray-200">{{ $choice->choice }}</label>
                                                </div>
                                            @endforeach
                                        </div>
                                        ...
                                        <div class="flex flex-col gap-4 items-center ps-4 rounded ">
                                            @foreach($question->choices as $choice)
                                                <div class="flex-1 w-full border border-slate-700 px-4 py-2">
                                                    <input id="choices[{{$question->id}}]" type="checkbox" value="{{$choice->id}}" name="choices[{{$question->id}}]" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2">
                                                    <label for="choices[{{$question->id}}]" class="w-full py-4 ms-2 text-sm font-medium text-gray-200">{{$choice->choice}}</label>
                                                </div>
                                            @endforeach
                                        </div>
